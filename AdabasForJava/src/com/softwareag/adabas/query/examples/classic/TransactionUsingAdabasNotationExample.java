 ...
 *
 * $Id: TransactionUsingAdabasNotationExample.java 6215 2017-08-24 12:16:07Z tkn $
 */
    ...
    public static void main(final String[] args) throws QueryException {
        ...
        AdabasConnection connection =
            AdabasConnection.createSession("acj:target=" + dbid
                + ";auth=NONE,user=" + ADA_SESSION_USER + ";IMPLICIT=false");
        ...
        try (ReadRequest request = connection.createReadRequest(ADA_FILENR)) {
            ...
            storeRequest.addStoredFields(
                Collections.singletonList(ADA_SHORT_FOR_SALARY));

            for (IRecordEntry record : list) {
                record.getDefinition().dumpFields(System.out);
                ((RecordEntry) record).dumpValues(System.out, true);
                ...
                long value = fieldSalary.longValue();

                /* Calculate new value */
                value += (value / 10);

                /* String representation of value */
                System.out.println(String.format("%4d %-20s %-20s %8d ->6 %8d",
                    record.getIsn(), fieldValueFirstName.toString(),
                    fieldValueName.toString(), fieldSalary.longValue(), value));
                storeRecord.addValue(ADA_SHORT_FOR_SALARY, value);
