 ...
 *
 * $Id: GenerateMappingsForLobExample.java 6220 2017-08-24 12:23:59Z tkn $
 */
...
/**
 * This Java application generates an example lob map in the Adabas Data Designer
 * file. The Adabas file number to store
 * Data Designer metadata is set to be 4 as default.
 */

public abstract class GenerateMappingsForLobExample {

    private static final int ADA_FILE_DATA_DESIGNER = 4;
    private static final int ADA_LOBDATA_FILE = 202;

    private static final String ADA_LOBMAP_MAPNAME = "LOB_MAP";

    /**
     * A lob example mapping from short to long name
     */
    private static final String[][] FIELD_DEFINITION_LOB_EXAMPLE =
                { { "BA", "Location" }, { "BC", "Filename" }, { "CA", "Type" },
                    { "CC", "MimeType" }, { "DA", "Data" }, { "DC", "Picture" },
                    { "EA", "Checksum" }, { "EC", "PictureSHAchecksum" } };

    /**
     * Defines name for a mapping configuration
     */
    private static final Object[][] MAP_DEFINITION = { { ADA_LOBMAP_MAPNAME,
        ADA_LOBDATA_FILE, FIELD_DEFINITION_LOB_EXAMPLE }, };
        ...
        try {
            ...
            if ("ACJ00064".equals(qe.getMessageCode())) {
                System.out.println("note: Please delete existing map entries "
                                + ADA_LOBMAP_MAPNAME + " using Data Designer");
            }
        }
    }
}
